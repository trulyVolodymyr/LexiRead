...
## Step 8 (later) — Deploy to production

The app is a static SPA — any static host works (Cloudflare Pages, Netlify, Vercel):

1. Build with the env vars set: `SUPABASE_URL` + `SUPABASE_KEY` in the host's environment, build command `npm run generate`, output dir `.output/public`.
2. Add your production URL to: Google OAuth **origins** (Step 3), Supabase **Site URL + Redirect URLs** (Step 4), and `APP_ORIGIN` secret (Step 5b).
3. Replace the placeholder icons `public/icon-192.png` / `public/icon-512.png` with real artwork so the installed PWA looks right.
