...
onMounted(async () => {
  await openAt(props.initial?.chunkIndex ?? 0, props.initial?.charOffset ?? 0, props.initial?.anchorY)
  setupSentinels()
})

/** Open the chapter containing chunkIndex, starting the lazy window at that chunk. */
async function openAt(chunkIndex: number, charOffset: number, anchorY?: number) {
  ...
  try {
    ...
    restoreTo(target, charOffset, anchorY)
  } finally {
    shifting = false
  }
}
...
async function jumpTo(chunkIndex: number, charOffset: number, anchorY?: number) {
  interacted = true
  await openAt(chunkIndex, charOffset, anchorY)
  emitPosition()
}
