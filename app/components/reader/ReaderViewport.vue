...
onMounted(async () => {
  await loadWindowAround(props.initial?.chunkIndex ?? 0, props.initial?.charOffset ?? 0, props.initial?.anchorY)
  setupSentinels()
})

async function loadWindowAround(chunkIndex: number, charOffset: number, anchorY?: number) {
  ...
  loading.value = false
  await nextTick()
  restoreTo(index, charOffset, anchorY)
}

/** Public: jump to an arbitrary position (TOC, restore after font change). */
async function jumpTo(chunkIndex: number, charOffset: number, anchorY?: number) {
  interacted = true
  shifting = true
  await loadWindowAround(chunkIndex, charOffset, anchorY)
  shifting = false
  emitPosition()
}
...
function restoreTo(chunkIndex: number, charOffset: number, anchorY?: number) {
  const root = chunkRoot(chunkIndex)
  if (!root || !container.value) return
  scrollToCharOffset(container.value, root, charOffset, anchorY ?? 8)
}
...
function currentPosition(): Position | null {
  ...
  // exact on-screen y of the anchor char, so a restore reproduces this viewport
  const caretRange = document.createRange()
  caretRange.setStart(caret.node, caret.offset)
  caretRange.collapse(true)
  const rect = caretRange.getBoundingClientRect()
  const anchorY = rect.top !== 0 || rect.height !== 0
    ? rect.top - container.value.getBoundingClientRect().top
    : undefined
  return { chunkIndex: index, charOffset, percent, anchorY }
}

// Positions generated by restore/window churn must never overwrite the saved
// place — only report after the user has actually interacted this session.
let interacted = false
const markInteracted = () => {
  interacted = true
}
useEventListener(container, 'wheel', markInteracted, { passive: true })
useEventListener(container, 'touchstart', markInteracted, { passive: true })
useEventListener(container, 'pointerdown', markInteracted)
useEventListener(window, 'keydown', markInteracted)
...
useEventListener(document, 'visibilitychange', () => {
  if (document.visibilityState !== 'hidden' || !interacted) return
  const position = currentPosition()
  if (position) emit('position', position)
})
